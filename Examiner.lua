...
function ex:InspectReady(guid)
	if (self:ValidateUnit()) then
		...
		if (info.guild) then
			...
			info.guildPoints, info.guildMembers = C_PaperDollInfo.GetInspectGuildInfo(unit);		-- WoD: Guild leveling removed. Az: What are guildPoints?
		end
...
function ex.ItemButton_OnClick(self,button)
	if (self.link) then
		local _, itemLink = C_Item.GetItemInfo(self.link);
		if (button == "RightButton") then
			AzMsg("---|2 Gem Overview for "..itemLink.." |r---");
			for i = 1, 3 do
				local gemLink = C_Item.GetItemGemProperties(self.link, i);
				if (gemLink) then
...
function ex.ItemButton_OnEnter(self,motion)
	...
	if (self.link) and (IsShiftKeyDown()) and (IsAltKeyDown()) then
		local itemName, _, itemRarity = C_Item.GetItemInfo(self.link);
		local is = LibItemString:New(self.link);
		gtt:AddLine(itemName,C_Item.GetItemQualityColor(itemRarity));
