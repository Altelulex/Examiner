...
function mod:UpdateItemSlots()
	for index, button in ipairs(self.slotBtns) do
		...
		if (not link) then
			...
			local _, _, itemRarity, itemLevel, _, _, _, _, _, itemTexture = C_Item.GetItemInfo(link);
			if (itemTexture) then
				button.texture:SetTexture(itemTexture or ITEM_ICON_UNKNOWN);
				local r,g,b = C_Item.GetItemQualityColor(itemRarity and itemRarity > 0 and itemRarity or 0);
...
local function OnClick(self,button)
	...
	elseif (self.link) then
		if (button == "RightButton") then
			AzMsg("---|2 Gem Overview for "..select(2,C_Item.GetItemInfo(self.link)).." |r---");
			for i = 1, 3 do
				local _, gemLink = C_Item.GetItemGem(self.link,i);
				if (gemLink) then
