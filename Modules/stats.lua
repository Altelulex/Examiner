...
local StatEntryOrder = {
	{ [0] = PLAYERSTAT_BASE_STATS, "STR", "AGI", "STA", "INT", "SPI", "ARMOR", "MASTERY", "VERSATILITY", "LIFESTEAL", "AVOIDANCE" },
...
local function GetGemAndItemInfo()
	...
	for slotName, link in next, ex.info.Items do
		...
		for i = 1, MAX_NUM_SOCKETS do
			...
			if (gemLink) then
				gemCount = (gemCount + 1);
				local _, _, _, _, _, _, itemSubType = C_Item.GetItemInfo(gemLink);
		...
		if (slotName ~= "TabardSlot") and (slotName ~= "ShirtSlot") then
			local _, _, itemRarity, itemLevel = C_Item.GetItemInfo(link);
