...
function UpdateShownItems(self)
	...
	for i = 1, NUM_BUTTONS do
		...
		if (link) then
			local itemName, _, itemRarity, itemLevel, _, _, _, _, itemEquipLoc, itemTexture = C_Item.GetItemInfo(link);
...
--			btn.name:SetTextColor(GetItemQualityColor(itemRarity or 0));
			...
			for i = 1, MAX_NUM_SOCKETS do
				local obj = btn["Gem"..i];
				if (gemTable[i]) then
					local itemName, _, _, _, _, _, _, _, _, itemTexture = C_Item.GetItemInfo(gemTable[i]);
					if (itemName) then
						obj.link = gemTable[i];
						obj.missing = nil;
